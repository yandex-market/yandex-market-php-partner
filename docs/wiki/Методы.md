Ниже представлено описание и примеры запросов к API и методов, которыми они реализованы. В примерах указаны только обязательные аргументы методов. Остальные аргументы описаны в статьях о соответствующих запросах технической документации партнерского API:
...
> **Внимание.** Методы для обработки заказов доступны только для модели DBS (продажи с доставкой продавца).
...
Клиент `OrderProcessingFromMarketClient` передает заказ магазину, запрашивает информацию о товарах в заказе. В ответ магазин должен отправить нужные данные и подтвердить принятие заказа.
...
> **Внимание.** Запрос отправляет Маркет.
...
$ordersObject = $orderProcessingClient->getOrders(10003, 12345);
...
> **Внимание.** Сейчас Маркет не принимает к размещению по модели DBS (продажи с доставкой продавца) товары, подлежащие маркировке в [системе «Честный ЗНАК»](https://честныйзнак.рф/). Вы можете поддержать метод, чтобы подготовиться к продаже таких товаров позже.
...
// Печатаем информацию о товарах, подлежащих маркировке
$items = $transferCisResponse->getItems();
foreach ($items as $item) {
    print_r($item->getId());
    print_r($item->getOfferId());
    print_r($item->getPrice());
    print_r($item->getVat());
    print_r($item->getInstances());
```
...
> **Внимание.** Методы, связанные с финансами, доступны только для модели ADV (реклама).
...
> **Внимание.** Методы, связанные со статистикой, доступны только для модели ADV (реклама).
...
> **Внимание.** Методы, связанные с контролем качества, доступны только для модели ADV (реклама).
